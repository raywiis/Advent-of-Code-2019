...
fn mock_execute(p: &mut Vec<i64>, i: &Vec<i64>, out: &mut Vec<i64>) {
...
impl Robot {
    ...
    fn cycle(&mut self) -> ExitCode {
        ...
        if self.last_idx + 2 != idx {
            // println!("{:?}, {}, {}, {}, {}, {:?}",
            //     outputs, self.position_x, self.position_y, color, turn, self.map.len());
            panic!("Fail man");
        }
        self.last_idx = idx;

        // println!("{}, {}, {}, {}, {}, {:?}",
        //     outputs.len(), self.position_x, self.position_y, color, turn, self.map.len());
...
fn main() {
    ...
    program.append(&mut [0; 10000].to_vec());
    ...
    let mut pixels: Vec<((i64, i64), i32)> = robot.map.iter()
